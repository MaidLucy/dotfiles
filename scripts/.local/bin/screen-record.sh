...
ffmpeg -y -hide_banner -framerate 120 -crtc_id $crtc_id -vsync 0 -device /dev/dri/card0 -vaapi_device /dev/dri/renderD128 -f kmsgrab -i - -vf 'hwmap,scale_vaapi=format=nv12' -c:v h264_vaapi -rc_mode 1 -qp 13 -profile:v 100 -level 62 -f mpegts osu-video-"$time".mpegts &
pids+=($!)
